...
public class BookGrpcServiceImpl extends BookServiceGrpc.BookServiceImplBase {
    ...
    @Override
    public void getBookByIsbn(Isbn request, StreamObserver<BookId> responseObserver) {
        logger.info("Grpc call recived: " + request.getIsbn());
        BookIdDto bookIdDto =  bookRepository
                .getBookByIsbn(request.getIsbn())
                .map(book -> new BookIdDto(book.getId(),book.getIsbn()))
                .orElseThrow(() -> new BookNotFoundException("Book could not found by isbn:" + request.getIsbn()));
        responseObserver.onNext(BookId.newBuilder()
